...
export async function getToolStatistics() {
  ...
  const powerStats = powers.length > 0 ? {
  ...
  const efficiencyStats = efficiencies.length > 0 ? {
  ...
  return {
    ...
    toolsByType: Object.entries(toolsByType).map(([type, toolList]) => ({
      ...
      avgEfficiency: toolList.length > 0 ? Math.round(toolList.reduce((sum, t) => sum + t.efficiency, 0) / toolList.length) : 0
    })),
