  ...
  for (const item of collectibleItems) {
    ...
    if (collectibleData) {
      ...
      const hasKnowledgeRequirements =
        collectibleData.requiredKnowledgesToUse.length > 0 || collectibleData.requiredKnowledgesToConvert.length > 0
...
export async function getCollectibleStatistics() {
  ...
  collectibles.forEach((collectible) => {
    typeDistribution[collectible.collectibleTypeName] = (typeDistribution[collectible.collectibleTypeName] || 0) + 1
  })
  ...
  const maxEquipStats =
    maxEquipCounts.length > 0
      ? {
          minEquipCount: Math.min(...maxEquipCounts),
          maxEquipCount: Math.max(...maxEquipCounts),
          avgEquipCount:
            Math.round((maxEquipCounts.reduce((sum, count) => sum + count, 0) / maxEquipCounts.length) * 100) / 100
        }
      : {
          minEquipCount: 0,
          maxEquipCount: 0,
