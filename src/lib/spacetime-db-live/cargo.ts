...
function isMaterialType(tag: string): boolean {
  const materialTags = [
    'Timber', 'Chunk', 'Ore Chunk', 'Geode', 'Roots',
    'Brick Slab', 'Frame', 'Sheeting', 'Tarp', 'Trunk'
  ]
...
export async function getCargoStatistics() {
  ...
  cargo.forEach((cargoItem) => {
    categoryDistribution[cargoItem.cargoCategory] = 
      (categoryDistribution[cargoItem.cargoCategory] || 0) + 1
  })

  // Calculate volume distribution
  const volumeDistribution: Record<string, number> = {}
  cargo.forEach((cargoItem) => {
    volumeDistribution[cargoItem.volumeCategory] = 
      (volumeDistribution[cargoItem.volumeCategory] || 0) + 1
  })
  ...
  const volumeStats = volumes.length > 0 ? {
  ...
  return {
    ...
    cargoByTag: Object.entries(cargoByTag).map(([tag, cargoList]) => ({
      ...
      avgVolume: cargoList.length > 0 ? 
        Math.round(cargoList.reduce((sum, c) => sum + c.volume, 0) / cargoList.length) : 0
    })),
    cargoByCategory: Object.entries(cargoByCategory).map(([category, cargoList]) => ({
      category,
      count: cargoList.length,
      avgTier: cargoList.length > 0 ? 
        Math.round(cargoList.reduce((sum, c) => sum + c.tier, 0) / cargoList.length) : 0,
      avgVolume: cargoList.length > 0 ? 
        Math.round(cargoList.reduce((sum, c) => sum + c.volume, 0) / cargoList.length) : 0
    })),
