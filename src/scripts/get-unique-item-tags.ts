...
function extractUniqueTags(items: unknown[]): string[] {
  ...
  for (const item of items) {
    ...
    if ((item as any).compendium_entry !== true) continue
...
function writeConstantFile(root: string, tags: string[]): void {
  ...
  const enumLines = tags
    .map((tag) => `  ${toEnumKey(tag)} = ${JSON.stringify(tag)},`)
    .join('\n')
