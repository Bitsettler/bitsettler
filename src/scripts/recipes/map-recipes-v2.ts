...
function loadLookupTables(
  ...
  try {
    ...
    const serverItemsPath = path.join(
      sourceDir,
      "server/region/item_desc.json"
    );
    const serverCargoPath = path.join(
      sourceDir,
      "server/region/cargo_desc.json"
    );
    const serverResourcesPath = path.join(
      sourceDir,
      "server/region/resource_desc.json"
    );
    const serverItemListPath = path.join(
      sourceDir,
      "server/region/item_list_desc.json"
    );

    const serverItems: ServerItem[] = JSON.parse(
      fs.readFileSync(serverItemsPath, "utf-8")
    );
    const serverCargo = JSON.parse(fs.readFileSync(serverCargoPath, "utf-8"));
    const serverResources = JSON.parse(
      fs.readFileSync(serverResourcesPath, "utf-8")
    );
    const serverItemLists: ServerItemList[] = JSON.parse(
      fs.readFileSync(serverItemListPath, "utf-8")
    );
    ...
    function findSlugByName(
      name: string,
      convertedData: { name: string; slug?: string }[]
    ): string | null {
    ...
    for (const serverItem of serverItems) {
      ...
      if (slug) {
        ...
        console.log(
          `✅ Mapped item: ${serverItem.name} (ID: ${serverItem.id}) -> ${slug}`
        );
      } else {
        console.log(
          `⚠️  No match found for item: ${serverItem.name} (ID: ${serverItem.id})`
        );
      }
    }
    ...
    for (const serverCargoItem of serverCargo) {
      ...
      if (slug) {
        ...
        console.log(
          `✅ Mapped cargo: ${serverCargoItem.name} (ID: ${serverCargoItem.id}) -> ${slug}`
        );
      } else {
        console.log(
          `⚠️  No match found for cargo: ${serverCargoItem.name} (ID: ${serverCargoItem.id})`
        );
      }
    }
    ...
    for (const serverResource of serverResources) {
      ...
      if (slug) {
        ...
        console.log(
          `✅ Mapped resource: ${serverResource.name} (ID: ${serverResource.id}) -> ${slug}`
        );
      } else {
        console.log(
          `⚠️  No match found for resource: ${serverResource.name} (ID: ${serverResource.id})`
        );
      }
    }

    console.log(
      `📊 Built item lookup with ${Object.keys(itemLookup).length} mappings`
    );
    console.log(
      `📊 Built item ID lookup with ${
        Object.keys(itemIdLookup).length
      } mappings`
    );
    console.log(
      `📊 Built item list lookup with ${
        Object.keys(itemListLookup).length
      } mappings`
    );
...
function convertRecipe(
  ...
  const professionSlug = profession
    ? lookups.professionLookup[profession]
    : "unknown";
  ...
  const buildingSlug = buildingType
    ? lookups.buildingLookup[buildingType]
    : undefined;
  ...
  try {
    ...
    for (const serverRecipe of serverRecipes) {
      try {
        ...
        processedCount++;
        console.log(
          `✅ Converted: ${serverRecipe.name} (ID: ${serverRecipe.id})`
        );
      } catch (error) {
        errorCount++;
    ...
    fs.writeFileSync(
      config.outputFile,
      JSON.stringify(frontendRecipes, null, 2),
      "utf-8"
    );
